...
class SanitizersTest < Minitest::Test
  ...
  def test_video_poster_sanitization
    scope_allowed_tags(%w(client)) do
      scope_allowed_attributes %w(src poster) do
        assert_sanitized %(<client src="videofile.ogg" autoplay  poster="posterimage.jpg"></client>), %(<client src="videofile.ogg" poster="posterimage.jpg"></client>)
        assert_sanitized %(<client src="videofile.ogg" poster=javascript:alert(1)></client>), %(<client src="videofile.ogg"></client>)
      end
    end
  end
